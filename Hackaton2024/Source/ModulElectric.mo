...
model ModulElectric "Model to aproximate charger and battery circuit"
  ...
  Modelica.Electrical.Batteries.BatteryStacks.CellStack cellStack(cellData = cellData, SOC(start = 1, fixed = true)) "battery powering the heater" annotation(
  ...
  Modelica.Electrical.Batteries.BatteryStacks.CellStack cellStack1(cellData = cellData1, SOC(start = 0.5, final fixed = true)) "battery powering the charger ciucuit" annotation(
  ...
  BooleanPulseLimited booleanPulseLimited1(startTime = 700, dutyCycle = 0.3, period = 1000, nPulses = 15) "module governing the behaviour of the charger circuit"  annotation(
    Placement(transformation(origin = {-72, -40}, extent = {{-10, -10}, {10, 10}})));
  BooleanPulseSuperpolated booleanPulseSuperpolated(startTime = 100, dutyCycleShort = 0.5, periodShort = 100, nPulsesShort = 5000, dutyCycleLong = 0.49, periodLong = 1000, nPulsesLong = 15) "module governing the behaviour of the heater circuit"  annotation(
...
<p>
This model was developed to simulate electric portion of the coffee maker. It consists of two parts, charger and battery circuit. Behaviour of both parts is governed by generated pulses. Battery circuit is simplified into 'cellStack', 'switch' and three serially connected resistors, representing 'Converter', 'ConverterLosses' and 'HeatingTube'. Last two resistors use heat ports for connections to the rest of model.
</p>
</html>"));
end ModulElectric;
